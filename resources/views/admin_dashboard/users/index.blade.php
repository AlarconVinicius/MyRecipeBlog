				...
				<div class="card">
					<div class="card-body">
						<div class="d-lg-flex align-items-center mb-4 gap-3">
						  ...
						  <div class="ms-auto"><a href="{{ route("admin.users.create") }}" class="btn btn-primary radius-30 mt-2 mt-lg-0"><i class="bx bxs-plus-square"></i>Add Usuário</a></div>
						</div>
								...
								<tbody>
                                    @foreach($users as $user)
                                        <tr>
                                            ...
                                            <td>
												<img style='width: 50px' src="{{ asset($user->image ? 'storage/' . $user->image->path : 'storage/placeholders/user_placeholder.png' . '') }}" alt="Foto de Perfil">
											</td>
                                            <td>{{ \Str::limit($user->full_name, 30) }}</td>
                                            <td>{{ \Str::limit($user->email, 50) }}</td>
                                            <td>{{ $user->role->nome }}</td>
											<td>
												<a class="btn btn-primary btn-sm" href="{{ route('admin.users.show', $user) }}">Posts Relacionados</a>
											</td>
